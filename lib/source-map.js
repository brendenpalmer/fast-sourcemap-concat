...
class SourceMap {
  ...
  addFile(filename) {
    const resolvedFilename = this._resolveFile(filename);

    // it's possible for certain `app.import`s to not exist; eg, if a given addon
    // imports a file within a lazy engine, where said addon also exists as dependency
    // as part of a more common host. if `ember-engines` dedupe is enabled this can
    // cause an error condition without this check (since in this case, it's expected
    // for this file to not exist within the lazy engine bundle)
    if (!this.fs.existsSync(resolvedFilename)) {
      return;
    }

    let source = ensurePosix(this.fs.readFileSync(resolvedFilename, 'utf-8'));
